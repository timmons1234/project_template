...
The purpose of this study is to understand factors that have contributed to the skill gap in Network Engineering for individuals new to the field (i.e. recent university graduates) and those currently working in the field as well as to determine if Modeling and Simulation training techniques can contribue to decreasing the skill gap.  
...
The first area to consider is to determine the training objective to help narrow the scope for the training system.  With a defined objective and scope the level of immersion, realism and interaction mechanics can be researched to determine the combination that most effectivly meets the training objective (Goodwin et al., 2015).

The next question to ask to answer is how the training will be accessed, i.e. online, a standalone applicaiton etc..  
